...
Thank you for helping keep AsterForge and its users safe. AsterForge is self-hosted software, so security reports are most useful when they clearly describe the affected version, deployment shape, and practical impact.

## Supported Versions

AsterForge is still evolving quickly. Security fixes are handled for the active development line and the latest tagged release.
...
- Email: `report@esaps.net` with a subject starting with `[AsterForge Security]`

Include as much of the following as you reasonably can:

- affected AsterForge version, git commit, or container image tag
...
Security reports are in scope when they affect AsterForge itself, including:
...
- server-side request forgery, request smuggling, unsafe redirects, deserialization, command execution, SQL injection, migration issues, or unsafe dependency usage that is exploitable in AsterForge
...
The following are usually out of scope unless they demonstrate a concrete exploit against AsterForge:
...
- generic dependency CVEs with no reachable or practical impact in AsterForge

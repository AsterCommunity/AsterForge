...
impl DbHandles {
    ...
    pub async fn close(self) -> std::result::Result<(), sea_orm::DbErr> {
...
mod tests {
    ...
    fn sqlite_urls_without_query_default_to_rwc_mode() {
        assert_eq!(
            normalize_database_url("sqlite:///var/lib/asterforge/app.db"),
            "sqlite:///var/lib/asterforge/app.db?mode=rwc"
        );
        assert_eq!(
            normalize_database_url("sqlite://data/asterforge.db"),
            "sqlite://data/asterforge.db?mode=rwc"
        );
    }
    ...
    fn sqlite_memory_and_existing_queries_are_preserved() {
        ...
        assert_eq!(
            normalize_database_url("sqlite:///var/lib/asterforge/app.db?mode=ro"),
            "sqlite:///var/lib/asterforge/app.db?mode=ro"
        );
        assert_eq!(
            normalize_database_url("postgres://user:pass@localhost/asterforge"),
            "postgres://user:pass@localhost/asterforge"
        );
    }
    ...
    fn sqlite_reader_url_forces_read_only_mode() {
        assert_eq!(
            super::sqlite_reader_url("sqlite:///var/lib/asterforge/app.db?mode=rwc"),
            "sqlite:///var/lib/asterforge/app.db?mode=ro"
        );
        assert_eq!(
            super::sqlite_reader_url("sqlite:///var/lib/asterforge/app.db?cache=shared"),
            "sqlite:///var/lib/asterforge/app.db?mode=ro&cache=shared"
        );
        assert_eq!(
            super::sqlite_reader_url("sqlite:///var/lib/asterforge/app.db?"),
            "sqlite:///var/lib/asterforge/app.db?mode=ro"
        );
        assert_eq!(
            super::sqlite_reader_url("sqlite:///var/lib/asterforge/app.db?&cache=shared"),
    ...
    async fn sqlite_reader_pool_is_query_only() {
        let url = format!(
            "sqlite:///tmp/asterforge-reader-pool-{}.db?mode=rwc",
            uuid::Uuid::new_v4()
        );
        let cfg = DatabaseConfig {
            url,
    ...
    async fn sqlite_reader_pool_reads_while_writer_connection_is_busy() {
        let url = format!(
            "sqlite:///tmp/asterforge-reader-writer-split-{}.db?mode=rwc",
            uuid::Uuid::new_v4()
        );
        let cfg = DatabaseConfig {
            url,

    ...
    async fn exchange_callback(
        ...
        let token_response = token_request
            ...
            .set_redirect_uri(Cow::Owned(
                RedirectUrl::new(callback.redirect_uri.clone()).map_external_auth_err_ctx(
...
pub(super) async fn build_client(
    ...
    let redirect_uri = RedirectUrl::new(redirect_uri.to_string()).map_external_auth_err_ctx(
...
pub(super) fn profile_from_id_token(
    ...
    if let Some(email) = email.as_deref() {
        aster_forge_validation::email::validate_email(email).map_err(|_| {
            ExternalAuthError::auth_invalid_credentials("OIDC email claim is invalid")
        })?;
    }

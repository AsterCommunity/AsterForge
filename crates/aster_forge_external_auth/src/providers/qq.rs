...
impl ExternalAuthProviderDriver for QqProviderDriver {
    ...
    async fn test_provider(
        ...
        if provider.client_id.trim().is_empty() {
            return Err(ExternalAuthError::validation_error("client_id is required"));
        }
        let provider = qq_oauth2_config(provider);
        let authorization_url = provider.authorization_url.as_deref().ok_or_else(|| {
            ExternalAuthError::validation_error("QQ authorization URL is missing")
        })?;
        let token_url = provider
            .token_url
            .as_deref()
            .ok_or_else(|| ExternalAuthError::validation_error("QQ token URL is missing"))?;
        let userinfo_url = provider
            .userinfo_url
            .as_deref()
            .ok_or_else(|| ExternalAuthError::validation_error("QQ userinfo URL is missing"))?;
        validate_url(
            authorization_url,
            "authorization_url",
            ExternalAuthError::validation_error,
        )?;
        validate_url(token_url, "token_url", ExternalAuthError::validation_error)?;
        validate_url(
            userinfo_url,
            "userinfo_url",
            ExternalAuthError::validation_error,
...
async fn exchange_qq_code_for_token(
    ...
    let token_url = provider
        .token_url
        .as_deref()
        .ok_or_else(|| ExternalAuthError::config_error("QQ token URL is missing"))?;
    let mut token_url = validate_url(token_url, "token_url", ExternalAuthError::config_error)?;
    {
    ...
    if token_response.access_token.trim().is_empty() {
        return Err(ExternalAuthError::auth_invalid_credentials(format!(
...
async fn fetch_qq_userinfo(
    ...
    let userinfo_url = provider
        .userinfo_url
        .as_deref()
        .ok_or_else(|| ExternalAuthError::config_error("QQ userinfo URL is missing"))?;
    let mut userinfo_url = validate_url(
        userinfo_url,
        "userinfo_url",
...
fn qq_openid_url(provider: &ExternalAuthProviderConfig) -> Result<reqwest::Url> {
    ...
    if token_url == QQ_TOKEN_URL {
        return validate_url(QQ_OPENID_URL, "openid_url", ExternalAuthError::config_error);
    }
    let parsed = validate_url(token_url, "openid_url", ExternalAuthError::config_error)?;

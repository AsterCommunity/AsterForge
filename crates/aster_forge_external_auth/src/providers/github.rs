...
impl ExternalAuthProviderDriver for GitHubProviderDriver {
    ...
    async fn test_provider(
        ...
        let authorization_url = provider.authorization_url.as_deref().ok_or_else(|| {
            ExternalAuthError::validation_error("GitHub authorization URL is missing")
        })?;
        let token_url = provider
            .token_url
            .as_deref()
            .ok_or_else(|| ExternalAuthError::validation_error("GitHub token URL is missing"))?;
        let userinfo_url = provider
            .userinfo_url
            .as_deref()
            .ok_or_else(|| ExternalAuthError::validation_error("GitHub userinfo URL is missing"))?;
        validate_url(
            authorization_url,
            "authorization_url",
...
fn select_verified_primary_email(emails: Vec<GitHubEmail>) -> Result<Option<String>> {
    ...
    aster_forge_validation::email::validate_email(&email).map_err(|_| {
        ExternalAuthError::auth_invalid_credentials("GitHub primary email is invalid")
    })?;
    Ok(Some(email))
}

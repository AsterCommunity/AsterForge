...
//!
//! Microsoft uses OIDC with tenant-aware issuer handling. This module normalizes tenant selectors,
//! builds Microsoft discovery URLs, accepts Microsoft multi-tenant issuer templates where
//! appropriate, and validates token issuers against the configured tenant policy.
...
fn microsoft_effective_issuer_url(provider: &ExternalAuthProviderConfig) -> Result<String> {
    ...
    normalize_microsoft_tenant_or_issuer_url(Some(issuer_url.to_string()))?
        .ok_or_else(|| ExternalAuthError::validation_error("Microsoft issuer URL is missing"))
}
...
async fn exchange_microsoft_callback(
    ...
    let nonce = callback
        .nonce
        .ok_or_else(|| ExternalAuthError::database_operation("stored OIDC nonce is missing"))?;
    let pkce_verifier = callback.pkce_verifier.clone().ok_or_else(|| {
    ...
    let id_token = token_response.extra_fields().id_token().ok_or_else(|| {
        ExternalAuthError::auth_invalid_credentials("OIDC token response missing id_token")
    })?;
    let verifier = CoreIdTokenVerifier::new_public_client(
        ClientId::new(provider.client_id.clone()),
        IssuerUrl::new(provider.require_issuer_url()?.to_string()).map_external_auth_err_ctx(
            "invalid Microsoft issuer URL",
...
async fn test_microsoft_provider(
    ...
    let token_endpoint = metadata.token_endpoint().ok_or_else(|| {
        ExternalAuthError::validation_error("OIDC discovery metadata missing token_endpoint")
    })?;
...
fn microsoft_discovery_url(configured_issuer: &str) -> Result<reqwest::Url> {
    let issuer = reqwest::Url::parse(configured_issuer).map_external_auth_err_ctx(
        "invalid Microsoft issuer URL",
        ExternalAuthError::validation_error,
    )?;
...
fn validate_microsoft_discovery_issuer(
    ...
    let configured = reqwest::Url::parse(configured_issuer).map_external_auth_err_ctx(
...
fn normalize_microsoft_issuer_url(value: &str) -> Result<String> {
    ...
    let parsed = reqwest::Url::parse(normalized).map_external_auth_err_ctx(
        "invalid Microsoft issuer URL",
        ExternalAuthError::validation_error,
    )?;
...
fn microsoft_tenant_from_issuer_url(parsed: &reqwest::Url) -> Result<String> {
    let segments = parsed
        .path_segments()
        .ok_or_else(|| ExternalAuthError::validation_error("Microsoft issuer URL missing tenant"))?
        .collect::<Vec<_>>();
...
fn microsoft_tenant_from_issuer_string(value: &str) -> Result<String> {
    let parsed = reqwest::Url::parse(value).map_external_auth_err_ctx(
        "invalid Microsoft issuer URL",
        ExternalAuthError::validation_error,
    )?;
...
fn validate_microsoft_token_issuer(configured_issuer: &str, token_issuer: &str) -> Result<()> {
    ...
    let token = reqwest::Url::parse(token_issuer).map_external_auth_err_ctx(
        "invalid Microsoft token issuer",
        ExternalAuthError::auth_invalid_credentials,
    )?;
    if configured.scheme() == "http" && configured.host_str().is_some_and(is_loopback_host) {
        return Err(ExternalAuthError::auth_invalid_credentials(
    ...
    let configured_tenant = microsoft_tenant_from_issuer_url(&configured)?;
    let token_tenant = microsoft_tenant_from_issuer_url(&token).map_err(|_| {
        ExternalAuthError::auth_invalid_credentials("Microsoft token issuer is invalid")
    })?;
    if is_microsoft_token_tenant_allowed_for_configured_tenant(&configured_tenant, &token_tenant) {
        return Ok(());
    }
    Err(ExternalAuthError::auth_invalid_credentials(
        "OIDC issuer does not match configured provider",
    ))
}
...
fn validate_microsoft_tenant(tenant: &str) -> Result<()> {
    ...
    Err(ExternalAuthError::validation_error(

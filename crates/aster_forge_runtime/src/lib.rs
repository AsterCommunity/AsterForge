...
//! coordination, shutdown phase coordination, and termination signal handling.
//! Product crates still own runtime state, audit events, background task
//! shutdown order, database handles, and service-specific readiness checks.
#![cfg_attr(
    not(test),
    deny(
        clippy::unwrap_used,

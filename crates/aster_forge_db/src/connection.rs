...
use crate::{DbError, Result, retry};
...
pub async fn connect_reader_for_writer(
    ...
    connect_reader_for_writer_with_metrics(cfg, writer, Arc::new(NoopDbMetrics)).await
}
...
fn install_db_metrics(db: &mut DatabaseConnection, metrics: SharedDbMetricsRecorder) {
    if !metrics.enabled() {
        return;
    }
...
mod tests {
    ...
    async fn sqlite_memory_handles_use_single_connection() {
        ...
        let writer = super::connect_with_metrics(&cfg, Arc::new(NoopDbMetrics))
            .await
            .expect("sqlite memory writer should connect");
        let handles =
            super::connect_reader_for_writer_with_metrics(&cfg, writer, Arc::new(NoopDbMetrics))
                .await

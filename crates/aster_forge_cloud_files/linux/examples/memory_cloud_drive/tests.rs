    ...
    fn test_directory(name: &str) -> PathBuf {
        ...
        async fn write_acceptance_uploads_exact_immutable_bytes_and_clears_current_dirty_state() {
            ...
            assert_eq!(
                store
                    .read_snapshot(commit.snapshot(), 3, 9)
                    .await
                    .unwrap(),
            ...
            assert_eq!(
                completed.session().map(ContentUploadSession::accepted_offset),
                Some(17)
            );
            assert_eq!(
        ...
        async fn remote_upload_commit_before_local_outcome_recovers_after_restart() {
            ...
            let store = Arc::new(
                MemoryWritebackStore::new(scope.clone(), Some(&directory)).unwrap(),
            );

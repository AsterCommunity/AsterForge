---
layout: home
description: AsterForge 公开文档首页，覆盖快速开始、运行时、配置、认证、审计后台任务、模板生成和 Docker 部署。

hero:
  name: AsterForge
  text: 可复用服务地基
  tagline: Rust + React 构建，把认证、运行时配置、审计日志、后台任务、OpenAPI、管理面板和部署默认值先做对，再让下游项目专注业务域。
  actions:
    - theme: brand
      text: 快速开始
...
5. 按 [审计与后台任务](./guide/audit-tasks.md) 给新增功能补审计记录和任务展示。
6. 用 [Docker 部署](./deployment/docker.md) 做最小生产化部署。

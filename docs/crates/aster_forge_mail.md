...
- 拉取可 claim 的 rows；
- 对每行按 `id` 执行 claim；
- 成功 claim 后把 row move 给产品传入的 deliver；
- deliver 成功后用 `retry_mark_sent` 尽力标记 sent；
- deliver 失败后按 `MailOutboxRetryPolicy` 决定 retry / failed，并按 `id` 标记状态；
- 调用产品传入的 audit callback，callback 收到轻量 `MailOutboxDispatchContext`；
- 返回统一的 `DispatchStats`。

如果产品使用 `aster_forge_db` 的共享 `mail_outbox` 表，优先调用
`MailOutboxDbStore::dispatch_due(...)`，不要直接拼这些 callback。直接接入
`dispatch_mail_outbox(...)` 主要留给非 Forge DB store 或定制 outbox persistence。
这类高级接入仍然由产品侧保留具体 entity/repository、claimable 查询条件、timestamp 计算、
模板渲染、audit 写入和产品错误类型。
dispatcher 不会 clone 完整 row；它只会在 deliver 前捕获 `id`、attempt、template code、收件人地址和可选收件人名称，避免复制可能较大的 payload JSON。
...
impl aster_forge_mail::MailOutboxDispatchRow for mail_outbox::Model {
    ...
    fn to_name(&self) -> Option<&str> { self.to_name.as_deref() }
}
```
...
- AsterYggdrasil：模板 payload enum、模板渲染、业务 URL 和发信审计 hook 留在产品代码里；sender、dispatch stats、投递失败 decision、重试策略、截断、`mark_sent` 重试、模板 registry、placeholder 渲染和 HTML/text 转换走 Forge。使用 Forge `mail_outbox` 表时，claim/retry/sent/failed 状态机通过 `aster_forge_db::MailOutboxDbStore::dispatch_due(...)` 接入。

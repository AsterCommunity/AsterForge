...
- `schedule`：`ScheduledTaskStore`、`ScheduledPeriodicTask`、`run_scheduled_periodic_task`。
...
- 如果产品已经使用 `aster_forge_runtime::AsterRuntime` component 模式，优先使用 `background_task_component()` 或 `background_task_component_with_descriptors()` 注册 shutdown，不要在产品侧手写 `component_shutdown_once("background_tasks", ...)`。
- 只有还没接入 component 模式的旧入口才需要直接调用 `shutdown().await`。
...
let background_tasks = spawn_runtime_background_tasks(state, shutdown_token);
...
如果产品没有使用 `runtime_task_registry!`，也可以继续用
`background_task_component_with_descriptors(...)` 或直接调用
`register_background_task_descriptors(...)`。已经有 `RuntimeTaskDefinition` 目录时，优先用
`background_task_component_with_definitions(...)`，不要在产品侧再写一层只循环
`wire_value/display_name` 的 registrar。

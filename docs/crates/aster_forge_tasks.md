...
- 如果产品已经使用 `aster_forge_runtime::AsterRuntime` component 模式，优先使用 `background_task_component()` 或 `background_task_component_with_definitions()` 注册 shutdown，不要在产品侧手写 `component_shutdown_once("background_tasks", ...)`。

...
aster_forge_db_migration = { git = "https://github.com/AsterCommunity/AsterForge", package = "aster_forge_db_migration" }
...
| `aster_forge_db_migration` | 无 | 无 | Migration 并发协调、跨数据库 index/search helper、主键和 UTC/JSON column builder；产品保留 migration list 和历史兼容策略。 |
...
`aster_forge_db::sort::SortOrder` 与 `aster_forge_api::SortOrder` 是同一个类型。API 层解析出的排序方向可以直接传给 DB sort helper。`pagination::fetch_offset_page` 会通过 `E: From<DbError>` 返回产品错误类型，因此产品 repository 不需要复制 pagination helper 或为每个调用点手写 `map_err`。
